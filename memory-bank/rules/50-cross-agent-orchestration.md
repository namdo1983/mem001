...
- Codex: root `AGENTS.md` should point to the installed harness entrypoint, such as `00-HARNESS.md` or `memory-bank/00-HARNESS.md`.
- Gemini: root `GEMINI.md` should point to the installed harness first.
- Claude/Cline: root `CLAUDE.md` should point to the installed harness. Keep active root `.clinerules/` absent unless a specific tool requires it.
- Antigravity-style agents: route through the shared `AGENTS.md`/`GEMINI.md` path unless a future tool explicitly needs a dedicated adapter.
